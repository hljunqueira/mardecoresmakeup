...
export default function AdminReservationsAndCredit() {
  ...
  // Mutação para adicionar produto à conta existente
  const addProductToAccountMutation = useMutation({
    mutationFn: async ({ accountId, productId, productName, quantity, unitPrice }: {
      accountId: string;
      productId: string;
      productName: string;
      quantity: number;
      ...
      // 2. Depois adicionar à conta
      const addResponse = await fetch(`/api/admin/credit-accounts/${accountId}/add-product`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          productId,
      ...
      if (!addResponse.ok) throw new Error('Erro ao adicionar produto à conta');
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: `${productAdded.quantity}x ${productAdded.productName} adicionado à conta. Novo total: ${formatCurrency(newTotalAmount)}`,
        duration: 5000,
      });
    },
    onError: (error) => {
      ...
      toast({
        ...
        description: "Ocorreu um erro ao adicionar o produto à conta. Tente novamente.",
        variant: "destructive",
      });
    },
  });
  ...
  // Função para abrir a interface de crediário com produtos selecionados
  const openCreditInterfaceWithProducts = (selectedProductsData: Array<{product: Product, quantity: number}>) => {
    if (selectedProductsData.length === 0) return;
    
    // Calcular valores totais
    const totalQuantity = selectedProductsData.reduce((sum, item) => sum + item.quantity, 0);
    const totalAmount = selectedProductsData.reduce((sum, item) => {
      return sum + (item.quantity * parseFloat(item.product.price));
    }, 0);
    
    // Criar um "produto virtual" representando o kit
    const kitProduct = {
      id: 'virtual-kit',
      name: `Kit de ${selectedProductsData.length} produto${selectedProductsData.length > 1 ? 's' : ''}`,
      price: (totalAmount / totalQuantity).toFixed(2),
      stock: Math.min(...selectedProductsData.map(item => item.product.stock || 0)),
      description: `Kit contendo: ${selectedProductsData.map(item => `${item.quantity}x ${item.product.name}`).join(', ')}`,
      category: 'Kit',
      brand: 'Diversos',
      images: [],
      featured: false,
    ...
    // Usar a interface existente
    handleStockReduction(kitProduct, (kitProduct.stock || 0) - totalQuantity);
  };

  // Função para abrir a interface de adicionar produtos a uma conta existente
  const openAddProductsToAccount = (account: CreditAccount, customer: Customer) => {
    setCurrentAccountForProducts({ account, customer });
  ...
  return (
    <div className="flex h-screen bg-background">
                          ...
                          <div key={account.id} className={`border-2 rounded-xl p-6 transition-all duration-200 hover:shadow-lg ${
                            ...
                            <div className="flex flex-wrap gap-3">
                              ...
                              <Button 
                                ...
                                onClick={() => customer && openAddProductsToAccount(account, customer)}
                              >
                                <ShoppingCart className="h-4 w-4 mr-2" />
                                Adicionar Produtos
                              </Button>
      ...
      {/* Stock Reduction Confirmation Modal - Reutilizando a interface existente */}
      ...
      {/* Modal para Seleção de Produtos - Atualizado para suportar adição a contas existentes */}
        ...
        <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center space-x-2">
              ...
              <span>
                {currentAccountForProducts 
                  ? `Adicionar Produtos - ${currentAccountForProducts.customer.name}`
                  : 'Selecionar Produtos para Crediário'
                }
              </span>
            </DialogTitle>
            <DialogDescription>
              {currentAccountForProducts 
                ? `Escolha produtos para adicionar à conta ativa de ${currentAccountForProducts.customer.name}`
                : 'Escolha um produto e quantidade para abrir a interface de crediário'
              }
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="py-4">
            {currentAccountForProducts && (
              <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                <h4 className="font-medium text-blue-900 mb-1">Conta Ativa:</h4>
                <p className="text-sm text-blue-700">
                  {currentAccountForProducts.account.accountNumber} - Valor atual: {formatCurrency(currentAccountForProducts.account.totalAmount || 0)}
                </p>
              </div>
            )}
            ...
            {productsLoading ? (
              ...
              <div className="space-y-3 max-h-60 overflow-y-auto">
                {products?.filter(product => (product.stock || 0) > 0).map((product) => (
                  <div 
                    key={product.id} 
                    className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50 cursor-pointer transition-colors"
                    onClick={() => {
                      if (currentAccountForProducts) {
                        // Lógica para adicionar à conta existente
                        const unitPrice = parseFloat(product.price);
                        addProductToAccountMutation.mutate({
                          accountId: currentAccountForProducts.account.id,
                        ...
                        // Lógica para nova conta
                        openCreditInterfaceWithProducts([{ product, quantity: 1 }]);
                      }
                      setIsProductSelectorOpen(false);
                      setCurrentAccountForProducts(null);
                    }}
                  >
                    <div className="flex-1">
                      <h4 className="font-medium">{product.name}</h4>
                      <p className="text-sm text-gray-500">{formatCurrency(product.price)}</p>
                      <p className="text-xs text-gray-400">Estoque: {product.stock || 0}</p>
                    </div>
                    <div className="text-right">
                      <Button size="sm" variant="outline">
                        {currentAccountForProducts ? 'Adicionar' : 'Selecionar'}
                      </Button>
                    </div>
                  </div>
                ))}
                {products?.filter(product => (product.stock || 0) > 0).length === 0 && (
